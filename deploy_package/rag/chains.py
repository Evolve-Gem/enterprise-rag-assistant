...
请基于以上资料生成结构化回答。"""
    ...
    try:
        ...
        return "调用 DeepSeek API 失败，请稍后重试或联系演示管理员。"
...
def generate_general_answer(question: str) -> str:
    """Generate a general answer when the knowledge base has no matching context."""
    question = question.strip()
    if not question:
        return "请输入问题。"
    ...
    system_prompt = (
        "你是企业知识库智能助手。当前没有检索到可引用的知识库资料，"
        "因此你只能给出通用建议，并必须明确说明这些内容未经过当前知识库验证。"
        "回答要简洁、结构清晰，适合技术销售或解决方案工程师参考。"
    )
    user_prompt = f"""用户问题：
{question}

请在开头明确说明“当前知识库未命中，以下为模型通用建议，未经过知识库资料验证”，
然后给出结构化回答。"""
